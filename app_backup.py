...
if st.session_state.active_repo is not None and not st.session_state.authenticated:
    ...
    users_exist = check_users_exist(st.session_state.active_repo)
            ...
            with st.form("register_form"):
                ...
                if register_btn:
                    if not reg_username or not reg_password:
                        ...
                        success, message = register_user(st.session_state.active_repo, reg_username, reg_password)
                        if success:
                            st.success(message)
        ...
        with st.form("login_form"):
            ...
            if login_btn:
                if not login_username or not login_password:
                    ...
                    success, message, user_data = login_user(st.session_state.active_repo, login_username, login_password)
                    if success:
        ...
        with st.form("reset_form"):
            ...
            if reset_btn:
                if not reset_username or not reset_new_password:
                    ...
                    success, message = reset_password(st.session_state.active_repo, reset_username, reset_new_password)
                    if success:
                        st.success(message)
                    else:
                        st.error(message)
